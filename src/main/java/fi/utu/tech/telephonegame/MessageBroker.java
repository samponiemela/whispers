...
public class MessageBroker extends Thread {
	...
	/**
	 public void omaViesti (Message viesti) {
	 prevMessages.put(viesti.getId());
	 }
	 **/

	private Message process(Object procMessage) {
		Message viesti = (Message) procMessage;
		if(prevMessages.containsKey(viesti.getId())){

			return null;
		}
		if (procMessage instanceof Message) {
			prevMessages.put(viesti.getId());
			gui_io.setReceivedMessage(viesti.getMessage());
			viesti.setMessage(Refiner.refineText(viesti.getMessage()));
			viesti.setColor(Refiner.refineColor(viesti.getColor()));
			gui_io.setSignal(viesti.getColor());
			gui_io.setRefinedMessage(viesti.getMessage());
	...
	public void run() {
		while (true) {
			try {
				Message viesti = process(procQueue.take());
				if (viesti != null) {
					network.postMessage(viesti);
				}
